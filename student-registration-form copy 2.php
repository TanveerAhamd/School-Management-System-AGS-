...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_student'])) {
    try {
        ...
        $stmt->execute([
            $reg, $_POST['admission_date'], $_POST['session_id'], $_POST['class_id'], $_POST['section_id'], $_POST['subject_group_id'],
            $_POST['student_name'], $_POST['cnic_bform'], $_POST['dob'], $_POST['gender'], $_POST['mother_language'], $_POST['cast'],
            $_POST['contact_no'], $_POST['address'], $_POST['guardian_name'], $_POST['relation'], $_POST['occupation'], $_POST['guardian_contact'],
            $_POST['prev_school_name'] ?? '', $_POST['last_class'] ?? '', $_POST['passing_year'] ?? '', $_POST['board_name'] ?? '',
            $_POST['disability'] ?? 'No', $_POST['hafiz_quran'] ?? 'No', $_POST['transport'],
            ($_POST['transport'] == 'Yes' ? $_POST['route_id'] : null),
            isset($_POST['interests']) ? implode(',', $_POST['interests']) : '', $_POST['remarks'] ?? '',
            $photo, $cnic_img, $gf, $gb, $rc
        ]);
...
<head>
  ...
  <style>
    ...
    .badge-subject { background: #6777ef; color: #fff; padding: 2px 8px; border-radius: 10px; margin: 2px; font-size: 11px; font-weight: bold; text-transform: uppercase; }
    .text-danger { color: red !important; }
    .doc-container { height: 100px; overflow: hidden; position: relative; display: flex; align-items: center; justify-content: center; border-radius: 4px; }
  </style>
</head>
              ...
              <div class="card mb-3">
                <div class="card-body py-2 b-0">
                  <div class="d-flex flex-wrap align-items-center justify-content-between">
                    <div class="mb-2 mb-md-0"><h5 class="page-title mb-0">Manage Student (Add)</h5></div>
                    <div>
              ...
              <div class="card-body rounded" style="border: 5px solid #0000004c !important ">
                        ...
                        <div class="text-center my-3">
                          <div class="border-primary d-flex gap-1 d-inline-block flex-wrap justify-content-around text-center">
                            ...
                            <div class="mt- px-2 d-flex align-items-end text-right">
                              ...
                              <select name="section_id" id="sel_section" class="form-select form-select-sm d-inline-block border-0 border-bottom rounded-0 p-0 text-center" style="width: 80px;" required><option value="">Section</option></select>
                            </div>
                            <div class="mt-0 px-2 d-flex align-items-end text-right">
                              ...
                              <select name="session_id" id="session_select" class="form-select form-select-sm d-inline-block border-0 border-bottom rounded-0 p-0 text-center" style="width: 80px;" required><option value="">Select</option></select>
                            </div>
                            <div class="mt-0 px-2 d-flex align-items-end text-right">
                          ...
                          <div class="mt-2"><div id="group_subjects_view" class="d-flex justify-content-around p-2 rounded border" style="background-color: #2727271e; min-height:40px;"></div></div>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div class="row mb-4">
                  <div class="col-12">
                    ...
                    <div class="row">
                      ...
                      <div class="col-md-3 mt-2"><label>Gender <span class="text-danger">*</span></label><select name="gender" class="form-control"><option value="Female" <?= (($_POST['gender']??'')=='Female'?'selected':'') ?>>Female</option><option value="Male" <?= (($_POST['gender']??'')=='Male'?'selected':'') ?>>Male</option></select></div>
                ...
                <div class="row mb-4">
                  <div class="col-12">
                    ...
                    <div class="row">
                      ...
                      <div class="col-md-3 mt-2"><label>Relation <span class="text-danger">*</span></label><select name="relation" class="form-control"><?php $rels=['Father','Mother','Uncle','Brother']; foreach($rels as $r) echo "<option value='$r' ".(($_POST['relation']??'')==$r?'selected':'').">$r</option>"; ?></select></div>
                ...
                <div class="row mt-4">
                  <div class="col-md-4 border rounded p-3">
                    <div class="d-flex justify-content-between mb-1"><label class="fw-bold">Disability:</label><div><input type="radio" name="disability" value="Yes" <?= (($_POST['disability']??'')=='Yes'?'checked':'') ?>> Yes <input type="radio" name="disability" value="No" <?= (($_POST['disability']??'No')=='No'?'checked':'') ?>> No</div></div>
                    <div class="d-flex justify-content-between mb-1"><label class="fw-bold">Hafiz-Quran:</label><div><input type="radio" name="hafiz_quran" value="Yes" <?= (($_POST['hafiz_quran']??'')=='Yes'?'checked':'') ?>> Yes <input type="radio" name="hafiz_quran" value="No" <?= (($_POST['hafiz_quran']??'No')=='No'?'checked':'') ?>> No</div></div>
                    <div class="d-flex justify-content-between mb-2 border-bottom pb-1"><label class="fw-bold">Transport:</label><div><input type="radio" name="transport" value="Yes" class="tr_toggle" <?= (($_POST['transport']??'')=='Yes'?'checked':'') ?>> Yes <input type="radio" name="transport" value="No" class="tr_toggle" <?= (($_POST['transport']??'No')=='No'?'checked':'') ?>> No</div></div>
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        <?php $ints = ['Cricket', 'Volleyball', 'Chess', 'Taekwondo', 'Scrabble', 'Skating'];
                          $posted_ints = $_POST['interests'] ?? []; if(!is_array($posted_ints)) $posted_ints = explode(',', $posted_ints);
                          foreach($ints as $i): ?>
                  ...
                  <div class="col-md-8 border rounded p-3">
                    ...
                    <div class="text-right mt-3"><div style="border-top: 2px solid #000; width: 200px; display: inline-block;"><p class="text-center fw-bold mb-0">Authority (Signature)</p></div></div>
                  </div>
                </div>
  ...
  <script>
    ...
    function previewManual(input, imgId, hidId) {
      if (input.files && input.files[0]) {
        ...
        reader.onload = e => { document.getElementById(imgId).src = e.target.result; document.getElementById(hidId).value = ""; };
    ...
    $(document).ready(function() {
      ...
      $('#sel_class').change(function() {
        $.getJSON('?action=fetch_sections&class_id=' + $(this).val(), function(data) {
          ...
          data.forEach(d => { let sel = (d.id == "<?= $_POST['section_id'] ?? '' ?>") ? 'selected' : ''; h += `<option value="${d.id}" ${sel}>${d.section_name}</option>`; });
          $('#sel_section').html(h);
        });
      }).trigger('change');
      ...
      $('.tr_toggle').change(function() {
        if ($(this).val() == 'Yes') {
          ...
          $.getJSON('?action=fetch_routes', function(data) {
            ...
            data.forEach(r => { let sel = (r.id == "<?= $_POST['route_id'] ?? '' ?>") ? 'selected' : ''; h += `<option value="${r.id}" ${sel}>${r.route_name}</option>`; });
            $('#sel_route').html(h);
          });
        } else { $('#sel_route').prop('disabled', true).val(''); }
      });
      if($('.tr_toggle:checked').val() == 'Yes') $('.tr_toggle').trigger('change');

      <?php if ($success): ?>
        Swal.fire('Saved!', 'Student Registered Successfully.', 'success').then(() => { window.location.href = 'student-registration-form.php'; });
      <?php elseif ($error): ?>
